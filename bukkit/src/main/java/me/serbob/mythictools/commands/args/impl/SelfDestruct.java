...
public class SelfDestruct implements CommandArgs {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        if (!player.hasPermission("mythictools.selfdestruct")) {
            player.sendMessage(ChatUtil.c("&cYou don't have permission to use this command!"));
            return false;
        }

        if (args.length < 2) {
            player.sendMessage(ChatUtil.c("&cUsage: /mythictools selfdestruct <time> or /mythictools selfdestruct -1 <time>"));
            return false;
        }
        ...
        if (args[1].equals("-1")) {
            if (args.length < 3) {
                player.sendMessage(ChatUtil.c("&cUsage: /mythictools selfdestruct -1 <time>"));
                return false;
            }
            isDelayed = true;
            timeStr = args[2];
