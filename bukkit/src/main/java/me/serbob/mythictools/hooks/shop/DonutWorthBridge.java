...
public class DonutWorthBridge implements ShopHook {
    ...
    public void init() {
        try {
            ...
            Bukkit.getLogger().info("[MythicTools] Successfully hooked into DonutWorth with " + priceCache.size() + " items cached!");
        } catch (ClassNotFoundException e) {
            Bukkit.getLogger().warning("[MythicTools] DonutWorth not found, hook disabled.");
        } catch (NoSuchMethodException e) {
            Bukkit.getLogger().warning("[MythicTools] DonutWorth API has changed, hook disabled.");
        } catch (Exception e) {
            Bukkit.getLogger().log(Level.SEVERE, "[MythicTools] Failed to hook into DonutWorth", e);
        }
    }
